...
<head>
  ...
  <style>
    ...
    td[onclick]:hover { background-color: #ffb; cursor: pointer; }
  ...
  <table class="table table-bordered table-condensed">
    ...
    <tbody>
      <c:forEach var="book" items="${ books }">
        <tr>
          <td onclick='location.href="edit?id=${ book.id }"'>${ book.id }</td>
          <td onclick='location.href="detail2?id=${ book.id }"'>${ book.title }</td>
          <td>${ book.author }</td>
          <td onclick='location.href="detail?id=${ book.categoryId }"'>${ book.categoryName }</td>
          <td>${ book.price }</td>
        </tr>
      </c:forEach>
    </tbody>
